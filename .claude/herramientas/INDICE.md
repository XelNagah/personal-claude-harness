...
| Base-0009 | preparar-worktree | Arma un worktree y le trae lo que git ignora —`settings.local.json` entre ellos, sin el cual el agente de adentro arranca sin ningún plugin y sin señal—; lo que está sin commitear no se copia. **Copia, nunca enlaza.** Cae en `.claude/tmp/worktrees/`, y a una raíz corta solo si la ruta no entra en el límite de Windows, diciendo por qué | script | `node .claude/herramientas/preparar-worktree/preparar-worktree.js <nombre> [--rama=<rama>] [--desde=<commit>] [--raiz-corta=<ruta>] [rutaRepo]` | vigente | [preparar-worktree/](preparar-worktree/) |
| Base-0010 | limpiar-worktree | Borra un worktree y verifica lo borrado. **No usa `git worktree remove`**, que atraviesa los junctions y vacía el destino real: borra con el borrado recursivo de Node y poda el registro. Frena si el agente escribió en el `.claude/` del worktree, y con `--forzar` respalda antes de borrar. `--listar` muestra los que hay | script | `node .claude/herramientas/limpiar-worktree/limpiar-worktree.js <nombre\|ruta> [--rama=borrar\|conservar] [--forzar] [rutaRepo]` | vigente | [limpiar-worktree/](limpiar-worktree/) |
| Base-0011 | worktrees | Calcula dónde cae un worktree, los lista, compara su `.claude/` contra el del repo y verifica el borrado; no crea ni borra ninguno. Única copia del repo: la usan las dos Herramientas del ciclo, y separa lo versionado —que se lo pregunta a git, porque el checkout convierte los fines de línea— de lo ignorado, que compara byte a byte | funcion | `require('../../common/worktrees.js')` | vigente | [../common/worktrees.js](../common/worktrees.js) |
