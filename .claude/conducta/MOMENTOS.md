...
| al cerrar tarea | Al terminar de responder una tarea. **Único momento donde hablar cuesta un turno completo del modelo**: emitir continúa la conversación en vez de dejar una nota, así que ahí callar es el default y el texto fijo de una regla `Inyectar` sale solo si una regla `Bloquear` del mismo momento lo habilita. Con `stop_hook_active` el repartidor sale mudo, o el agente no puede cerrar. | `Stop` | activo |
