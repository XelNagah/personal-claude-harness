...
- [Semántica](feedback_semantica.md) — terminología del dominio en `.claude/semantica/`; dos registros pares (glosario legítimo + Terminología Farlopa de relaciones vetadas); el veto es término→significado, no el término; consultar al planificar/analizar.
...
- [Conducta](feedback_conducta.md) — reglas "cuando hagas X, asegurate de Y" en `.claude/conducta/`; atan momentos (evento de hook + condición sin juicio) a acciones (inyectar/correr/bloquear); las entrega un hook repartidor que lee el registro vivo, el agente no lo consulta a mano (una regla cargada al arranque se recita, no se obedece); Base vs Propósito; lint al cerrar.
