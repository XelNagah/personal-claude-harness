...
El detector del nivelador (`amp-actualizar.js`) sabe qué Componentes de Subsistema tiene que haber porque **están escritas a mano en su código**: la lista de subsistemas, `HERRAMIENTAS_BASE`, los Componentes de Subsistema de `conducta`, los tres eventos de hook, `identidad.md`. La plantilla de `amp:inicializar` tiene esa misma información, en otro formato.

Son **dos fuentes para el mismo dato**, y nada las mantiene sincronizadas. Cada Componente de Subsistema nuevo que sume la plantilla hay que acordarse de agregarla también al detector — y si no, el nivelador informa "al día" un repo al que le falta. Ya pasó dos veces el mismo día:
...
El cierre del 26/07/2026 mitiga el síntoma: `amp:inicializar` **no puede reportar sin correr el detector** y exigir cero pendientes. Eso ata las dos fuentes por comportamiento, pero la duplicación del dato sigue —y la tercera vez probó que el mitigante no cubre al nivelador, donde el cero del detector **es** la conclusión.
