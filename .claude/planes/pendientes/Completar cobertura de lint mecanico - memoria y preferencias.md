...
**Estado: en diseño · Creado 26-07-18.** Foco. Prerequisito del rework del README (para que describa el patrón completo, sin excepciones). Surgió en la sesión de `/planificar` del rework del README.
...
- **`memoria`** — tiene `MEMORY.md` + `.md` pero **no tiene lint**. Es el único subsistema de acumulación sin él.
...
`estilo-commits` no necesita lint propio: vive dentro de `memory/`, lo cubre `lint-memoria`. `setup-completo` y `planificar` quedan afuera: operacionales, no persisten estado.

## Alcance

### 1. `lint-memoria` (dentro de la funcionalidad `memoria-local`)
Calcado de `lint-conocimiento`: refs rotas, índice (`MEMORY.md`) incompleto, `.md` huérfanos, frontmatter faltante/ inválido (`name`/`description`/`metadata.type`).
