...
> **Comparable externo (2026-07-23).** **Hermes Agent (Nous Research)** materializa el frente C: gestiona su memoria con una herramienta (`memory_tool`) que el propio agente edita en el punto de acción (agregar/reemplazar/borrar, con chequeo de inyección de prompt antes de escribir), y dispara trabajo desatendido con una herramienta (`cronjob`) que corre en sesión nueva (incluso en "modo sin agente": script agendado, cero LLM). Es evidencia externa de que el mecanismo "en el punto de acción" es un patrón resuelto, no una apuesta. Relevamiento de mecanismos en el repo de mejora de uso: `D:\Proyectos\analisis\como-uso-claude\.claude\conocimiento\hermes-agent.md`. Salvaguarda a copiar: las tareas agendadas de Hermes **fallan cerrado** si el modelo/proveedor por defecto cambió (no gastan en silencio) — aplicable al disparo por hook/cron de acá.

### Ya ejecutado (21/07/2026): B mínimo — inventario sin rutear

Primera parte del frente B. Herramienta `inventariar-componentes-sueltos` (`.claude/herramientas/inventariar-componentes-sueltos/`): barre `.claude/` y lista los componentes (archivos y carpetas) que no son subsistema (por el criterio de lint co-ubicado de `pantalla-bienvenida`, decisión 0008) ni infra conocida. **Inventaría, no juzga.** Corre suelta (no cableada a `control-cierre`: eso ya sería veredicto) y acepta una ruta para apuntarla a un consumidor a mano.
