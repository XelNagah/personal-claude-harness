...
- **Cómo sabe que la sesión nueva terminó de arrancar.** Solo hace falta si el orden es lanzar→terminar. Candidatas: esperar a que exista el proceso hijo (débil: existir no es estar listo), una marca en disco que escriba un hook `SessionStart` de la sesión nueva (fuerte, pero suma una pieza que hay que instalar), o un tiempo de espera fijo (frágil). Tercera opción: que el orden elegido haga la pregunta innecesaria.
