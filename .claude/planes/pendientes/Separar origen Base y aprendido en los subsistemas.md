...
**Estado: Nuevo · Creado 26-07-24.** Precondición de la mitad `amp-actualizar` (nivelador) del plan de distribución marketplace. Surgió al analizar cómo el nivelador distingue lo que puede pisar de lo que no.
...
## Por qué es precondición del nivelador
...
- **Por fila** (conducta) → dos secciones, tabla propia cada una: `## Reglas Base` / `## Reglas del Propósito`. Molde: Base/Adaptaciones de `PREFERENCIAS.md`. El nivelador reemplaza la sección Base entera, nunca abre la del Propósito. **Ya ejecutado en este repo** (conducta partida el 24/07/2026).
- **Por archivo** (memoria) → los archivos Base (`feedback_*` que siembra el harness) tienen nombre propio; el nivelador los pisa por nombre, los aprendidos quedan intactos. La separación sale sola por nombre; el rediseño de fondo de memoria (prefijo legacy, corte con conocimiento) va en su propio plan.
- **Contenido puro aprendido** (glosario, semántica, conocimiento, decisiones, planes, herramientas) → el harness los instala vacíos; el nivelador de rutina **no reescribe** su contenido. Lo único Base es el mecanismo (lint, MANIFIESTO, estructura), archivos enteramente Base que el nivelador pisa completos. No hay mezcla adentro ⇒ nada que separar.

**Escape a futuro:** no se precluye despachar contenido sembrado en glosario/semántica/conocimiento; sería una migración dedicada, no tarea del nivelador de rutina.
