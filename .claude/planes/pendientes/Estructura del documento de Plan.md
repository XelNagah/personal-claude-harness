...
- **Concepto de "Plan".** ¿Qué es un plan en este repo? Candidato a entrada de glosario (el término es del usuario, no acuñado — pasa el gate). Distinguir del **registro** (`PLANES.md`) y del **estado**.
...
Si se define plantilla/esquema, toca: `funcionalidades/gestion-de-planes/` (PLANTILLA + prompt + skill), el orquestador `setup-completo` (verbatim), posible entrada de glosario, y quizá `lint-planes` si se chequea estructura.
