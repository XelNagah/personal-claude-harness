...
3. **El repartidor combina clases en un mismo momento:** junta el texto de las reglas `inyectar` con la salida de las `correr` cuando ambas producen `additionalContext` (`PreToolUse`, `UserPromptSubmit`); `SessionStart` sigue reenviando la salida tal cual. Hoy `correr` corta el despacho (`establecer-conducta.js:112`), así que sumar la detección **apagaría** el recordatorio existente. Se descartó que un solo script arme todo el mensaje: mudaría el texto de la Regla Base del registro al código, donde deja de leerse y de actualizarse.
