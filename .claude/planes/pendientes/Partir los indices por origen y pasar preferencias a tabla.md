...
- **El origen se declara en un frontmatter, no en el nombre del archivo.** El actualizador identifica qué reemplazar leyendo `origen`, así que el nombre queda libre para decir qué lista. Eso descarta `MULTIPROPOSITO.md` y sus variantes, que fue el punto donde el diseño se trabó: el nombre decía de dónde venía el contenido y ocultaba cuál era.
...
1. Partir los tres índices en dos archivos cada uno.
   ...
   **Ejecutado el 29/07/2026.** Son **cuatro**, no tres (`subsistemas` entró con el diseño de la 0042). Lo hecho y lo medido de más contra el diagnóstico:
   ...
   - **El actualizador detecta las dos migraciones** (frontmatter ausente, partición pendiente): 13 hallazgos contra una copia del repo anterior al cambio, 0 contra el repo al día.
...
2. **Estandarizar los diez Índices al núcleo** (era "pasar `preferencias` a tabla"; se ensanchó el 29/07/2026 — ver la sección de arriba). En orden:
   ...
   2. `preferencias` a tabla, con sus páginas de detalle. Es el único que hoy no es tabla junto con `conocimiento`.
      ...
      **`preferencias` entra al Patrón — 29/07/2026, misma sesión.** Dejó de ser la excepción:

      - **`MANIFIESTO.md` y `README.md`**, y con eso la excepción sale de tres lugares: `lint-subsistemas.js:123` (`&& fila.nombre !== 'preferencias'`), la sección propia de `AGENTS.md`, y la lista `SUBSISTEMAS` del actualizador, que tenía **siete** subsistemas en vez de ocho — o sea que a `preferencias` nunca le miró el manifiesto ni el lint.
      - **La cadena de importación pasa a tres saltos:** `AGENTS.md` → `MANIFIESTO.md` → los dos Índices. Antes `AGENTS.md` importaba los Índices directo desde su propia sección.
      - **`lint-preferencias` al Patrón.** Antes chequeaba tres cosas; ahora valida el núcleo de cada fila (prefijo del código según el origen, códigos sin repetir, Nombre único dentro del Índice, Descripción no vacía), las referencias de la columna `Detalle`, las páginas huérfanas y la cadena de importación entera. **Los once controles se probaron contra un caso bueno y uno malo cada uno.**
      - **De paso, el defecto del parseo de tuberías se arregló acá.** `lint-preferencias` separa las celdas respetando `\|`; los otros lints y el repartidor de conducta todavía no, y eso queda en el punto 2.7.
      - Propagado: los dos Índices, el manifiesto y el README como copias textuales, los dos lints, la sección de `AGENTS.md` en la plantilla, la habilidad instaladora y la nota de migración. Verificado con `lint-harness` y con el detector del actualizador contra un Agente Desplegado de prueba con la forma vieja, que reporta `preferencias/MANIFIESTO.md — ausente: instalar`.

      **Pendiente de este subpaso:** el actualizador detecta el manifiesto ausente pero **no** la conversión de bullets a tabla, ni el `README.md` ausente, ni la sección `## Preferencias` que hay que borrar de `AGENTS.md`. Los diecisiete Agentes Desplegados con la forma vieja necesitan esas tres detecciones antes de migrarse (punto 7 del trabajo).
   3. Los otros nueve al núcleo: sumar `Código`, `Nombre`, `Descripción` y `Detalle` donde falten, renombrar encabezados, ordenar las filas por Código y sumar el campo del frontmatter que declara qué representa la Descripción.
      ...
      **`semantica` hecho el 30/07/2026.** Los dos registros al núcleo: 33 conceptos y 39 relaciones vetadas, todos `Local-NNNN`, y Terminología Farlopa estrena la columna `Detalle` que no tenía. `Significado vetado` pasó a `Significado Farlopa`, que ahora vive en el frontmatter y en el texto que encabeza la tabla. **Cuatro roturas silenciosas, y una de ellas es la peor de la sesión:**
      ...
      Los cuatro ubican ahora por nombre de columna o por lo que el archivo declara de sí mismo, y aceptan la forma vieja mientras haya Agentes Desplegados sin actualizar. Versiones: `amp-semantica` 0.7.0, `amp-conducta` 0.4.0.
      ...
      **Dos roturas más del mismo tipo, y una es del repartidor:**
      ...
      Los dos aceptan `Nombre` y `Regla` mientras haya Agentes Desplegados sin actualizar. **Nueve controles probados contra un caso bueno y uno malo**, incluidos tres sobre el repartidor: que entregue el momento `cada turno`, que entregue la clase `Ejecutar` en `SessionStart`, y que lea también el Índice del Agente Desplegado.
   ...
   4. **Los lints y el resto del código que lee las tablas.** Se fue haciendo junto con cada subsistema, no como paso aparte: cada migración destapó su propia rotura.
      ...
      **El actualizador cerrado el 30/07/2026.** Le faltaban cinco detecciones, y sin ellas informaba "al día" un Agente Desplegado con la forma vieja. Ahora detecta:
