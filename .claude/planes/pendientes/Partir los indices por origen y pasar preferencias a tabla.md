...
2. **Estandarizar los diez Índices al núcleo** (era "pasar `preferencias` a tabla"; se ensanchó el 29/07/2026 — ver la sección de arriba). En orden:
   ...
   2. `preferencias` a tabla, con sus páginas de detalle. Es el único que hoy no es tabla junto con `conocimiento`.
      ...
      **`preferencias` entra al Patrón — 29/07/2026, misma sesión.** Dejó de ser la excepción:
      ...
      - **`MANIFIESTO.md` y `README.md`**, y con eso la excepción sale de tres lugares: `lint-subsistemas.js:123` (`&& fila.nombre !== 'preferencias'`), la sección propia de `AGENTS.md`, y la lista `SUBSISTEMAS` del nivelador, que tenía **siete** subsistemas en vez de ocho — o sea que a `preferencias` nunca le miró el manifiesto ni el lint.
      ...
      - Propagado: los dos Índices, el manifiesto y el README como copias textuales, los dos lints, la sección de `AGENTS.md` en la plantilla, la habilidad instaladora y la nota de migración. Verificado con `lint-harness` y con el detector del nivelador contra un Agente Desplegado de prueba con la forma vieja, que reporta `preferencias/MANIFIESTO.md — ausente: instalar`.
