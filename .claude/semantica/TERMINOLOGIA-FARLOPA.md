...
| Local-0008 | `Workflow` | un flujo de trabajo cualquiera (como encabezado o sustantivo); no el nombre propio de la herramienta de orquestación de la plataforma, que es legítimo | flujo de trabajo | avisa | — |
...
| Local-0029 | `reconciler` | el actualizador consolidado (`amp-actualizar`) | el actualizador (glosario: *Actualizar*) | bloquea | — |
...
| Local-0036 | `Herramientas Base` / `Reglas Base` / `preferencias Base` / `piezas Base` | etiqueta de origen de un grupo de registros. **`Base` sola no se veta**: la palabra es corriente (`base de conocimiento`) y nombra legítimamente la parte no-aprendida de un Agente con Propósito. | `Herramientas del Agente Multipropósito`, `Reglas del Agente Multipropósito`, … | avisa | — |
...
| Local-0042 | `nivelar` / `nivelador` / `nivelación` / `nivelado` | poner al día una instalación existente, o el mecanismo que lo hace | `actualizar` / `el actualizador` (glosario: *Actualizar*) | avisa | — |
| Local-0043 | `transcript` | el archivo donde queda registrada la conversación de la sesión | la transcripción de la sesión | avisa | — |
| Local-0044 | `sidechain` | la ejecución de un subagente, fuera del hilo principal de la sesión | en un subagente / fuera del hilo principal | bloquea | — |
| Local-0045 | `working tree` | los archivos del repo tal como están en disco, por oposición al índice y a la historia de git | los archivos en disco / el árbol de trabajo | bloquea | — |
| Local-0046 | `code review` | la revisión del código por otro —persona o agente— antes de integrarlo | revisión de código | bloquea | — |
| Local-0047 | `crux` | el punto decisivo de un análisis, lo que decide el resultado | el punto decisivo / el nudo / lo que decide | bloquea | — |
| Local-0048 | `just-in-time` | carga o entrega de algo en el momento en que se necesita | a demanda | avisa | — |
| Local-0049 | `empujar` / `empuje` / `empujón` | inducir una conducta en el agente o en el usuario, o el mecanismo que la induce | `recordar` / `pedir` / `un recordatorio` | avisa | — |
| Local-0050 | `copia de trabajo` | el directorio adicional que crea `git worktree add` | worktree | avisa | — |
| Local-0051 | `bundle` | el conjunto de plugins que se instalan juntos en un repo: «el bundle `amp`», «bundle completo, no à la carte» | paquete; o «se instala todo junto» cuando la frase describe la regla | avisa | — |

Ratificados por el usuario (barridos del 2026-07-19 y 2026-07-20; `dogfooding` el 2026-07-21; `tripa` y los anglicismos de mecanismo —`static`, `binding`, `dispatcher`, `catch-all`, `feasibility`, `stress-test`, `thin`— el 2026-07-22; `churn` y `wedge` el 2026-07-23; los cinco ex-`Vetados` del glosario —`artefacto`, `gate`, `prosa`, `levelear`, `verbatim`— migrados al registro el 2026-07-23; `reconciler`, `install-prompts`, `dogfood` y `reforma de disco` el 2026-07-24; `cruce` y `huevo-y-gallina` el 2026-07-24; `capa`=fase el 2026-07-25; `ciclo-de-plan` el 2026-07-26; las etiquetas de origen —`Herramientas Base` y sus hermanas, `Herramientas del Propósito`, `Reglas del Propósito` y `Adaptaciones`— el 2026-07-28; `pintar` el 2026-07-29; `nivelar` y sus formas el 2026-08-06; `crux` y `just-in-time` el 2026-08-07 (barridos del texto de conversación del propio agente, sin apariciones vivas en archivos); `transcript`, `sidechain`, `working tree` y `code review` el 2026-08-06, junto con la redefinición de `Workflow`, que pasó de vetar «cualquier uso» a vetar el sustantivo común y dejar afuera el nombre propio de la herramienta). Esos cuatro salieron de barrer **el texto que el propio agente produjo en una sesión** —sus respuestas y sus mensajes de commit—, un alcance que hasta entonces ningún control miraba. `payload` y `lookup` se evaluaron y **no** se vetaron: se entienden y se usan. `Base` sola se evaluó y **no** se vetó: es palabra corriente y además nombra legítimamente la parte no-aprendida de un Agente con Propósito; vetarla marcaba 673 apariciones, casi todas válidas, y un registro que marca todo entrena a ignorarlo. El texto vivo ya está barrido; este registro existe para que el lint cace **regresiones**.
...
⚠️ **`Local-0043`** es la tercera: `transcript` entra como `avisa` porque `transcript_path` es el nombre del campo del contrato de hooks de Claude Code y aparece legítimamente en el código. En texto plano el canónico es «la transcripción de la sesión», y quedan apariciones vivas por barrer en unos doce archivos. Sus tres hermanos de la misma tanda entran barridos: `sidechain` y `working tree` no tenían ninguna aparición en el repo, y las dos de `code review` se corrigieron el mismo día.

⚠️ **`Local-0042`** es la otra excepción, y la más grande: se vetó el 2026-08-06 y el texto vivo **todavía no está barrido**. `nivelar` se había colado como el término de poner al día una instalación existente, cuando el canónico del usuario siempre fue `actualizar` — el mismo verbo que ya nombraba la habilidad (`amp:actualizar`). Al ratificarlo quedaron **432 apariciones vivas en 137 archivos**, de las cuales **130 viajan dentro de los plugins** y **76 están en `base/`**, o sea instaladas en cada Agente Desplegado. El sustantivo `nivelador` es el 61% del total. Por eso la fila entra como `avisa` y no como `bloquea`: bloquear haría que editar cualquiera de esos 137 archivos se rechace, y el barrido no se podría hacer por partes. El barrido tiene plan propio.
