...
| Local-0013 | Controles que dejan de controlar sin avisar | Quince formas en que un control se apaga: valida un conjunto vacío, mira una copia, nadie lo probó, solo revisa lo que ya conoce, su banco lee el repo autor, o su excepción abarca todo el repo. | [controles-que-no-avisan.md](controles-que-no-avisan.md) |
...
| Local-0018 | Medir el ahorro de contexto de un subagente de subsistema | Leer la transcripción `.jsonl` del subagente, sumar sus `tool_result` contra el reporte devuelto y verificar el modelo ahí, no en el frontmatter. Medido: 84/94/89/97% de ahorro, y 137 s de duración. | [medir-subagentes-de-subsistema.md](medir-subagentes-de-subsistema.md) |
| Local-0019 | Las barras invertidas dobles se pierden al escribir código desde la herramienta de Bash | `\` llega como `\`, antes de que Bash vea el comando: no lo evitan ni las comillas simples ni un heredoc citado. No falla — una expresión regular así contó 282 donde debía contar 2. | [barras-invertidas-desde-bash.md](barras-invertidas-desde-bash.md) |
