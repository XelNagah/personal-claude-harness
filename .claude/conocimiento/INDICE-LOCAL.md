...
| Local-0003 | Hooks de Claude Code — referencia de mecánica | Los 9 eventos del núcleo, cuáles bloquean, el contrato de exit code y JSON, matchers y precedencia; incluye cómo resolver la raíz del repo desde un hook, que `CLAUDE_PROJECT_DIR` no da. | [hooks-claude-code.md](hooks-claude-code.md) |
...
| Local-0013 | Controles que dejan de controlar sin avisar | Las ocho formas en que un control se apaga solo y sigue en verde: valida sobre un conjunto vacío, se quedó sin población, mira una copia, nadie lo probó. Cómo se prueba un control. | [controles-que-no-avisan.md](controles-que-no-avisan.md) |
