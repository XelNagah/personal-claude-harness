...
| Base-0006 | El plan que reparte su trabajo en otros planes | Los frentes se resuelven en los hijos y nadie vuelve al padre: sus pendientes envejecen de tres formas, y la peor es la que se resolvió al revés de como el plan la pedía. | [el-plan-que-reparte-su-trabajo.md](el-plan-que-reparte-su-trabajo.md) |
| Base-0007 | Borrar un worktree con `git worktree remove` vacía el destino del enlace que tenga adentro | En Windows git no reconoce el junction: entra y borra del otro lado, y su `Permission denied` significa «borré hasta acá», no «no borré nada». El borrado recursivo de Node es inocente: no atraviesa. | [borrar-un-worktree-con-enlace-adentro.md](borrar-un-worktree-con-enlace-adentro.md) |
| Base-0008 | Comparar dos copias de un repo byte a byte en Windows las marca distintas aunque sean iguales | `core.autocrlf` viene en `true` desde la instalación y el checkout convierte los fines de línea: el mismo archivo mide 20 bytes de un lado y 22 del otro. No falla: contesta que cambió todo. | [comparar-dos-copias-de-un-repo.md](comparar-dos-copias-de-un-repo.md) |
