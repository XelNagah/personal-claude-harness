...
| Base-0014 | Entregar un handoff cuando el trabajo pasa a otro agente ahora | Un handoff traspasa trabajo a otro agente **en el momento**: el usuario copia un texto, abre una sesión y lo pega. No es para archivar ni para dejar algo para otro día — eso es un plan, y va al subsistema planes. Ante «dejalo para mañana» o «guardá esto», abrir o actualizar un plan, no escribir un handoff. Se escribe cuando el usuario lo pide o cuando hay que cortar porque el contexto se llenó. Va en `.claude/tmp/` con un nombre que diga **de qué es** (`handoff-<tema>.md`), nunca un nombre fijo, y lleva en este orden: el texto con que el receptor pone al usuario en tema, ya redactado para una persona que no leyó nada —qué se venía haciendo, dónde quedó y qué sigue— y que el receptor le entrega antes de trabajar, preguntar o nombrar un código; qué hacer primero al arrancar; cómo verificar el estado del repo con los comandos; qué quedó pendiente y en qué plan vive; y las trampas ya pagadas. **Escribir el archivo no completa el traspaso:** la respuesta que lo anuncia tiene que traer el texto para pegar, en un bloque de código de **hasta cinco líneas**, con la ruta del archivo adentro y **nada del contenido** — el que lo reciba abre el archivo. Un texto que haya que scrollear para copiar no sirve. Si el handoff era el pedido, la tarea termina ahí; si es para delegarle una parte a otro agente mientras esta sesión sigue, decir qué le tocó al otro y qué queda acá. | [handoff.md](handoff.md) |
...
| Base-0017 | Decir de qué componente es cada texto que se muestra para aprobar | Al mostrar un texto para que el usuario lo apruebe —el título o el cuerpo de una decisión, un mensaje de commit, una fila de registro, el nombre de una entrada—, decir antes **qué componente es** y **dónde va a quedar escrito**. Un bloque suelto obliga a adivinar contra qué convención juzgarlo, y las convenciones difieren entre componentes: el título de una decisión dice qué se decidió, el de un commit dice qué estaba mal antes. Aplica también cuando el texto se muestra por segunda vez o en un turno posterior al que lo introdujo. | — |
