...
description: Da de alta una funcionalidad/plugin nueva en el harness — carpeta con plugin.json/README/skill/prompt, marketplace.json, junction local, fila en REGISTRO.md, orquestador si aplica — y valida. Use when el usuario dice "agregá una funcionalidad", "creá el plugin", "sumá esta pieza al harness".
---
...
2. **Crear `funcionalidades/<nombre>/`:**
   ...
   - `skills/<nombre-skill>/SKILL.md` — versión Claude Code (`.claude/` literal); `PLANTILLA.md` aparte si lleva textos que se copian tal cual.
   - `prompt.md` — versión agnóstica (placeholder `<config>` para el directorio del harness).
   - Invariante: skill y prompt divergen en forma, no en contenido.
3. **Sumar el plugin a `.claude-plugin/marketplace.json`**: `name` + `source: "./funcionalidades/<nombre>"` (el `source` debe arrancar con `./`).
4. **Junction local** para edición en vivo (no mezclar con el plugin instalado en la misma máquina):
   ```powershell
   New-Item -ItemType Junction -Path "$env:USERPROFILE\.claude\skills\<nombre-skill>" -Target "<ruta-repo>\funcionalidades\<nombre>\skills\<nombre-skill>"
   ```
5. **Registrar en `REGISTRO.md`**: fila en la tabla de funcionalidades + fila en la tabla Plugin/Skill.
6. **Orquestador** (solo si es de convención): sumarla a `setup-completo` — paso en SKILL/prompt + textos duplicados en su `PLANTILLA.md`. Respetar el orden de dependencias.
