...
4. **Cablear en `AGENTS.md`** (punto de entrada en la raíz, decisión 0010) por su **manifiesto** (decisiones 0017/0019):
   - **Crear `.claude/conocimiento/MANIFIESTO.md`** con el contenido de [PLANTILLA.md](PLANTILLA.md) §Manifiesto. Ya trae la regla de ubicación única, **el disparador de escritura** (*cuándo* asentar: al averiguar algo del dominio que costó descubrir, con la prueba «¿seguiría siendo cierto si este repo no existiera?»; la skill `registrar-conocimiento` hace el flujo) y el paso de lint al cerrar; e **importa su índice** con la línea final `@INDICE.md` (el índice de conocimiento se carga siempre). ⚠️ Sin el disparador nadie usa la carpeta y el subsistema queda en cero mientras el lint da verde (un índice vacío es coherente) — por eso el disparador va en el manifiesto, siempre en contexto.
   - **Asegurar la sección `## Subsistemas`** (PLANTILLA §Subsistemas; la crea `memoria-local`, o crearla si falta) y, dentro, la línea `@.claude/conocimiento/MANIFIESTO.md`.
   - **Migración (modelo viejo).** Si AGENTS.md ya tenía una sección de prosa "Base de conocimiento del proyecto" y/o la línea `@.claude/conocimiento/INDICE.md` en un bloque "Mapa del repo", el manifiesto las reemplaza: quitarlas. Si el bloque Mapa queda sin líneas de subsistema, quitar su encabezado.
5. **Migración (el punto de la funcionalidad).** Detectar conocimiento que viva **fuera** de `.claude/conocimiento/` y moverlo adentro. Buscar en **tres** lugares, no solo el obvio:
   ...
   **(c) Fuentes crudas — NO son conocimiento y NO se mueven.** Distinguir lo que el agente **lee** de lo que **sabe**: escaneos, PDFs de resúmenes/facturas, exports, json/csv de origen, adjuntos. Son insumos inmutables; se quedan donde están (salvo que ya estén entreverados dentro de una carpeta de conocimiento, en cuyo caso se mueven con ella). El conocimiento es el md **sintetizado**.
   ...
   - **Reparar referencias:** paths en cada `INDICE.md`, links entre páginas, refs desde `AGENTS.md` y desde las memorias/planes, y el acople de los scripts que se muevan a `herramientas/<tool>/` — `__dirname` (reapuntar a `.../conocimiento/...`) o **cwd** (prependerles `process.chdir(require('path').join(__dirname, '<ruta a los datos>'))`, que evita reescribir cada llamada de I/O).
