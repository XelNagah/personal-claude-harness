...
4. **Sección en `AGENTS.md`** (punto de entrada en la raíz, decisión 0010). Asegurar una sección **"Base de conocimiento del proyecto"** con link a `.claude/conocimiento/INDICE.md`, la regla de ubicación única, **el disparador de escritura** (*cuándo* asentar, no solo dónde vive: al averiguar algo del dominio que costó descubrir, con la prueba «¿seguiría siendo cierto si este repo no existiera?»; la skill `registrar-conocimiento` hace el flujo) y el paso de lint al cerrar. ⚠️ Sin el disparador, la sección describe una carpeta y nadie la usa: el subsistema queda en cero mientras el lint da verde, porque un índice vacío es perfectamente coherente. Si ya hay una equivalente (otro título, mismo tema), no duplicar. Si existe el bloque **"Mapa del repo (siempre cargado)"** (de `memoria-local`), asegurar la línea `@.claude/conocimiento/INDICE.md` en él.
