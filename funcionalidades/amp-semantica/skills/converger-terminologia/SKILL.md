...
description: Recorre un texto contra los dos registros del subsistema semántica —el glosario y el registro de Deriva Semántica, alias Terminología Farlopa— y propone ratificar un alias, vetar o reescribir; también revisa si las filas de esos registros están bien puestas. El alcance se le indica al invocarla (todo el repo, los planes, lo que se publica, o un texto); sin indicación, todo el repo. Nada se asienta sin el usuario. Use when aparece un término del dominio sin registrar, aunque el usuario no pida ningún chequeo ni nombre la terminología: si propone un nombre para algo ("esto lo llamaría X"), si circula un anglicismo o una copia literal del inglés, o si se está por adoptar una palabra nueva en un registro, un plan o el nombre de un Componente de Subsistema. También cuando dice "converger terminología" o "revisá los términos". Disparar antes de usar el término propuesto — usarlo como si ya fuera canónico es lo que este subsistema existe para evitar.
---
