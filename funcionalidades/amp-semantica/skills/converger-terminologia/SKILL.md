...
description: Recorre un texto contra los dos registros del subsistema semántica —el glosario y el registro de Deriva Semántica, alias Terminología Farlopa— y propone ratificar un alias, vetar o reescribir; también revisa las filas de esos registros. El alcance se le indica al invocarla (todo el repo, los planes, lo que se publica, o un texto); sin indicación, todo el repo. Nada se asienta sin el usuario. Use when aparece un término del dominio sin registrar. SIEMPRE invocá esta skill cuando alguien proponga un nombre para algo ("esto lo llamaría X"), cuando circule un anglicismo o una copia literal del inglés, cuando se esté por adoptar una palabra nueva en un registro, un plan o el nombre de un Componente de Subsistema, o cuando el usuario diga "converger terminología" o "revisá los términos" — aunque no pida ningún chequeo ni nombre la terminología. No adoptes ni uses el término propuesto directamente — invocá esta skill primero: usarlo como si ya fuera canónico es lo que este subsistema existe para evitar.
---
