---
name: amp-actualizar
description: Nivela el .claude/ de un repo con el AMP ya instalado contra la plantilla nueva del harness. Converge por estructura, sin guardar versión: pisa lo Base (mecanismo del harness) respaldándolo antes en .claude/.respaldo-amp/<fecha>/, nunca toca lo aprendido (contenido del repo), y pregunta antes de reacomodar formas viejas. Aplica renombres conocidos (glosario→semantica) e instala subsistemas faltantes (conducta), delegando en los inicializar-<sub>. Trae vista previa. Use when el usuario dice "nivelá el AMP", "actualizá el harness del repo", "poné al día el .claude", "amp-actualizar", o al detectar un repo con el harness viejo.
---

# amp-actualizar — nivelador del harness

Pone al día el `.claude/` de un repo que **ya** tiene el AMP instalado, contra la plantilla nueva. **No** es para arrancar un repo de cero (para eso está `inicializar-custom`): es para uno vivo, sin romperle lo que aprendió. Diseño en la decisión 0028; se apoya en la separación Base/aprendido (decisión 0027).
...
- **Skill** (este flujo): confirma el plan, **delega la instalación** de cada subsistema a su `inicializar-<sub>` (que trae la plantilla 0024-limpia), migra términos y prosa con criterio, y pregunta ante lo divergente.
...
5. **Aplicar Base** (el grupo Base del plan). Para cada ítem:
   - **Subsistema ausente** (p. ej. `conducta/`) → correr su `inicializar-<sub>` (instala completo desde su PLANTILLA).
   - **`MANIFIESTO`/lint/estructura vieja** → tomar el contenido canónico de la PLANTILLA del `inicializar-<sub>` y **pisar** el archivo Base. (A diferencia de la reconciliación normal de los `inicializar-*`, que preserva lo existente, acá el archivo Base **se pisa** — es del harness. El contenido aprendido del mismo subsistema no se toca.)
...
6. **Aplicar Renombres** (el caso con más juicio — preservar lo aprendido):
   - **`glosario`→`semantica`:**
     ...
     2. Correr `inicializar-semantica` en reconciliación: pone al día el mecanismo (lint nuevo, `MANIFIESTO`, estructura de columnas) **preservando** `GLOSARIO.md` y `TERMINOLOGIA-FARLOPA.md` con sus términos. Verificar que ningún término se haya perdido.
...
Todo lo que se instala o pisa en el consumidor sale de las PLANTILLAs de los `inicializar-<sub>`, que ya enuncian la razón inline sin citar números de decisión del harness. El nivelador no introduce números de decisión en el `.claude/` del consumidor.
