...
Da de alta en el registro (`.claude/comunicacion/INDICE.md`) otra instalación del Agente Multipropósito de la misma máquina, para poder consultarla después con `consultar-agente`. **Solo se registran otras instalaciones del Agente Multipropósito**, no agentes cualquiera: se reconocen porque tienen su `.claude/` con el harness instalado.
...
1. **Reunir los cuatro datos.** Preguntar lo que falte:
   ...
   - **CLI** — `claude` (Claude Code) o `codex` (Codex CLI). Decide cómo se arma el comando de consulta.
2. **Validar antes de escribir.**
   ...
   - El **CLI** es uno soportado (`claude` o `codex`). Otro no se puede invocar en solo lectura: no registrar y decirlo.
