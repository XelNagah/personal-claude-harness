...
Instala el sistema de preferencias del usuario: separado por origen en **dos archivos**, cada uno declarado en su frontmatter — `preferencias/PREFERENCIAS.md` (`origen: agente-multiproposito`, el actualizado lo reemplaza entero) y `preferencias/PREFERENCIAS-LOCAL.md` (`origen: agente-desplegado`, las del usuario en este repo; el actualizado nunca lo abre) — los **dos importados siempre al contexto** vía `@` desde el archivo de instrucciones. Las preferencias son reglas de conducta: tienen que estar inline, no disponibles-a-pedido (nada dispara "ir a buscar" una regla que se está por violar).
