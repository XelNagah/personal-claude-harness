...
La presencia de `.claude/memoria/` significa **migración incompleta**, aunque todos sus archivos sean válidos para la versión vieja. Nunca responder “ya estaba al día” ni “nada para actualizar” mientras exista. El único cierre válido es uno de estos:
...
- **Skill** (este flujo): confirma el plan, usa `amp:inicializar` **solo para instalar Componentes ausentes**, pisa la Base vieja con las reglas de este actualizador, migra términos y textos con criterio, y pregunta ante lo divergente.

## Paso previo obligado: la fase de plugins

Poner al día un repo son **dos fases** y esta skill ejecuta la segunda. La primera —los plugins de la máquina— va **antes**, porque esta misma skill viaja adentro del plugin: si se actualizan los archivos con los plugins atrasados, el repo queda puesto al día por una versión vieja del instalador.
...
5. **Frenar y pedir el reinicio.** Los plugins nuevos no cargan hasta entonces. Al volver, el usuario pide `amp:actualizar` de nuevo y ahí sí se actualizan los archivos.
...
3. **Confirmar el plan.** Los **Divergentes** se preguntan uno por uno (son bloqueantes): no se toca nada de ese grupo sin ok explícito. Ejemplo típico: `conducta/INDICE.md` con reglas pero sin el corte Base/Propósito — repartirlas exige decidir cuáles son Base y cuáles del Propósito; eso lo decide el usuario.
   ...
   **`columna(s) fuera de la convención de la Base`** es el otro caso, y el que más cuidado pide: el encabezado del repo declara una columna que la Base no trae. Pisar el bloque dejaría cada fila con un dato de más bajo una cabecera que ya no lo nombra — el registro queda corrupto y sus lectores validan sobre una columna que desapareció.
   ...
   ⚠️ Una columna propia **sola** —sin que la convención de arriba haya cambiado y con las columnas de la Base en su lugar— **no genera hallazgo**: es Aprendizaje legítimo y no hay nada que actualizar. Si un repo con una columna propia apareciera marcado en cada corrida sin nada que hacer al respecto, eso es un defecto del detector, no del repo.
...
5. **Aplicar Base** (el grupo Base del plan). La fuente canónica es la carpeta `base/` de `amp:inicializar`: los Componentes de Subsistema son **archivos**, con el mismo árbol que ocupan en el destino.
   ...
   Para cada ítem:
   - **Subsistema ausente** (p. ej. `conducta/`) → correr `amp:inicializar` (idempotente: instala los Componentes ausentes y no actualiza los existentes).
   ...
   - **`settings.json` sin la clave `outputStyle`** → agregarla por merge en `.claude/settings.json`, con el valor **exacto** del `name` que declara el frontmatter del Estilo de Respuesta que viaja en `base/output-styles/` (`Español corriente`, con acento y mayúscula, o Claude Code la ignora). El archivo del estilo llega con el resto de la Base, pero un `.md` en `output-styles/` no hace nada hasta que esta clave lo nombra: sin el merge, el repo recibe el Componente **apagado** y no se entera. Es la misma lógica que el `SessionStart` que deja funcionando la Pantalla de bienvenida. **Solo Claude Code**: `.codex/hooks.json` no la lleva, porque Codex CLI no tiene equivalente y ahí el estilo degrada a nada sin fallar. Si el destino ya trae **otro** valor, no se toca: cae en divergente.
   - **`.gitignore` sin alguna de las rutas que el repo tiene que ignorar** → merge del bloque `§Gitignore` de la PLANTILLA de `amp:inicializar`, sin sacar nada de lo que ya esté. Son de dos clases y las dos se revisan en cada corrida, no solo al instalar lo que las trae: las rutas **donde escribe el mecanismo** —un repo instalado antes de que ese bloque existiera versiona el buzón de avisos y los borradores del agente, y los lints que excluyen `.claude/tmp/` de su barrido trabajan sobre una premisa que ese repo no cumple—, y el **Índice de `comunicacion`**, que guarda rutas absolutas de máquina. Ojo con el segundo: un repo que recibió ese subsistema en una versión anterior ya tenía ignoradas las otras rutas, así que nada lo marcaba y la línea no le llegaba nunca.
...
6. **Aplicar Renombres** (el caso con más juicio — preservar lo aprendido):
   - **`glosario`→`semantica`:**
     ...
     2. Aplicar sobre `semantica/` las reglas de actualización del punto 5: instalar lo ausente, reemplazar el mecanismo Base y migrar los Índices conservando sus términos. No delegar esta actualización a `amp:inicializar`, que solo completa ausencias. Verificar que ningún término se haya perdido.
   ...
   - **Índice sin el núcleo de columnas** (`tabla sin el nucleo del Indice`, `sin tabla`): el archivo puede estar declarado y partido por origen y aun así conservar la forma vieja de su tabla. Es **migración de contenido, no reemplazo**: el actualizador nunca la aplica solo.
...
Segura de re-correr: una segunda corrida sobre un repo ya actualizado da todo "ya estaba" y no escribe nada. Es el mismo comando para migrar (repo viejo) y reconciliar (repo al día): la vista previa dice cuál es el caso.

## 0024

Todo lo que se instala o pisa en el consumidor sale de `base/` y de la PLANTILLA de `amp:inicializar`, que ya enuncian la razón inline sin citar números de decisión del harness. El actualizador no introduce números de decisión en el `.claude/` del consumidor.
