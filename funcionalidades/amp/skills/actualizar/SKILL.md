...
5. **Aplicar Base** (el grupo Base del plan). La fuente canónica es la carpeta `base/` de `amp:inicializar`: los Componentes de Subsistema son **archivos**, con el mismo árbol que ocupan en el destino.
   ...
   - **`.gitignore` sin las rutas donde escribe el mecanismo** → merge del bloque `§Gitignore` de la PLANTILLA de `amp:inicializar`, sin sacar nada de lo que ya esté. Un repo instalado antes de que ese bloque existiera versiona el buzón de avisos y los borradores del agente, y los lints que excluyen `.claude/tmp/` de su barrido trabajan sobre una premisa que ese repo no cumple.
