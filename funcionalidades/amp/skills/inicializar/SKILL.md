...
- **`.claude/settings.json`** y **`.codex/hooks.json`** → el repartidor `establecer-conducta` en los tres eventos (`SessionStart`, `UserPromptSubmit`, `PreToolUse` con matcher `Write|Edit`) más el lint de planes en `SessionStart`, **sin sacar los hooks que ya estén**. En `settings.json` va además la clave `outputStyle`, que **enciende** el Estilo de Respuesta (`.claude/output-styles/`) recién copiado con el árbol `base/` — el Componente Base no se instala apagado, igual que la Pantalla de bienvenida queda funcionando. Solo se escribe si la clave falta; un `outputStyle` distinto ya presente no se pisa, se reporta divergente. Bloques y regla en `PLANTILLA.md` §Hooks.
