...
- **Clase** — quién recibe lo que la regla produce: `Inyectar` (un texto para el agente, que actúa con su juicio) · `Ejecutar` (un programa cuya salida ve el usuario) · `Controlar` (un Control que decide, y puede frenar la acción). Definidas en [`CLASES.md`](CLASES.md).
- **Contenido** — el dato que el hook consume: el texto a inyectar (`Inyectar`) o la ruta del programa a correr (`Ejecutar`, `Controlar`).
...
> **Origen del contenido:** las reglas se separan por origen en **dos archivos**, y cada uno lo declara en su frontmatter — este (`origen: agente-multiproposito`, las manda el Agente Multipropósito; el actualizador `amp:actualizar` lo reemplaza entero al poner al día un Agente con Propósito) e [`INDICE-LOCAL.md`](INDICE-LOCAL.md) (`origen: agente-desplegado`, las suma cada repo; el actualizador no lo abre). El repartidor lee los dos. Hoy tienen repartidor los momentos `al arrancar la sesión` (`SessionStart`, clase `Ejecutar`), `cada turno` (`UserPromptSubmit`), `al escribir` (`PreToolUse`) y `al cerrar tarea` (`Stop`).

> ⚠️ **En `al cerrar tarea` una regla `Inyectar` NO sale sola:** ahí el default es callar y hace falta una regla `Controlar` del mismo momento que la habilite. La excepción está explicada una sola vez, en [`MOMENTOS.md`](MOMENTOS.md#el-momento-donde-hablar-cuesta-un-turno).

## Reglas del Agente Multipropósito

Las que instala el Agente Multipropósito. El actualizador `amp:actualizar` reemplaza **este archivo entero** al poner al día un Agente con Propósito; nunca abre el del Agente Desplegado.
...
| Base-0006 | Frenar la terminología vetada antes de que se escriba | Rechaza la escritura que usa un término sin uso legítimo posible, e informa los que dependen del significado | al escribir | Controlar | conducta/detectar-terminologia-vetada/detectar-terminologia-vetada.js | vigente | — |
...
| Base-0009 | Registrar en el subsistema cuando algo cambia | Cierra la tarea derivando a su subsistema lo que otro debe saber | al cerrar tarea | Inyectar | Si en esta tarea cambió algo que otro subsistema debe saber (decisión, conocimiento, semántica, preferencia, herramientas, conducta o catálogo de subsistemas), registralo antes de cerrar. | vigente | — |
| Base-0010 | Proponer dejar la sesión limpia al terminar una tarea | Recuerda la disciplina de higiene de sesión al terminar una tarea: persistir lo que falte, dejar en su plan el trabajo que sigue otro día, y `/clear` con la terminal siempre abierta | cada turno | Inyectar | Si la tarea quedó terminada, persistida y verificada, proponé dejar la sesión limpia antes de la próxima: asentá lo que falte, actualizá su plan si el trabajo sigue otro día, escribí un handoff solo si otro agente lo retoma ahora (Preferencia Base-0014) y sugerí `/clear`. Aclarale al usuario que la terminal queda abierta y en escucha: se limpia el contexto, no la sesión. | vigente | — |
| Base-0011 | Avisar cuando el contexto de la sesión se puso pesado | Mide la transcripción de la sesión como aproximación del contexto y, pasado el umbral, propone un punto de corte; nunca frena nada | cada turno | Controlar | conducta/avisar-contexto-pesado/avisar-contexto-pesado.js --umbral 150000 | vigente | — |
| Base-0012 | Habilitar el aviso de cierre solo cuando la sesión no asentó nada | Decide si el aviso de cierre sale: acá emitir cuesta un turno del modelo, así que sale una vez por sesión y solo si hubo conversación de verdad y no se escribió ningún registro. Nunca frena | al cerrar tarea | Controlar | conducta/avisar-sesion-sin-asentar/avisar-sesion-sin-asentar.js --piso 200000 | vigente | — |
