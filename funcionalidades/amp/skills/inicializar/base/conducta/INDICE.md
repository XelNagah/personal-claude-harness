...
> ⚠️ **En `al cerrar tarea` una regla `Inyectar` NO sale sola.** Es el único momento donde emitir cuesta una vuelta completa del modelo —no existe dejar una nota y que el turno igual cierre—, así que el repartidor calla por default y el texto fijo sale solo cuando una regla `Bloquear` del mismo momento lo habilita. Una regla `Inyectar` ahí sin ninguna `Bloquear` que la habilite no se entrega nunca; el `lint-conducta` lo marca.
...
| Base-0010 | Proponer dejar la sesión limpia al terminar una tarea | Empuja la disciplina de higiene de sesión: tarea terminada → persistir + handoff + `/clear`, con la terminal siempre abierta | cada turno | Inyectar | Si la tarea quedó terminada, persistida y verificada, proponé dejar la sesión limpia antes de la próxima: asentá lo que falte, escribí un handoff si hay trabajo que retomar (Preferencia Base-0014) y sugerí `/clear`. Aclarale al usuario que la terminal queda abierta y en escucha: se limpia el contexto, no la sesión. | vigente | — |
| Base-0011 | Avisar cuando el contexto de la sesión se puso pesado | Mide la transcripción de la sesión como aproximación del contexto y, pasado el umbral, propone un punto de corte; nunca frena nada | cada turno | Bloquear | conducta/avisar-contexto-pesado/avisar-contexto-pesado.js --umbral 150000 | vigente | — |
| Base-0012 | Habilitar el aviso de cierre solo cuando la sesión no asentó nada | Decide si el aviso de cierre sale: acá emitir cuesta un turno del modelo, así que sale una vez por sesión y solo si hubo conversación de verdad y no se escribió ningún registro. Nunca frena | al cerrar tarea | Bloquear | conducta/avisar-sesion-sin-asentar/avisar-sesion-sin-asentar.js --piso 200000 | vigente | — |
