...
3. **Asegurar `planes/pendientes/`, `planes/ejecutados/`, `planes/descartados/`** (con `.gitkeep` si el repo usa git), **`planes/ESTADOS.md`** (verbatim de PLANTILLA §Estados) y **`planes/PLANES.md`** (semilla de PLANTILLA §Semilla). Si hay planes sin fila, agregarlas (ver migración).
4. **Asegurar las memorias** `feedback_flujo_planes.md` y `feedback_artefacto_estado.md` (verbatim de PLANTILLA) + sus líneas en `memoria/MEMORIA.md`.
5. **Instalar el lint**: `planes/lint-planes/lint-planes.js` + `README.md` (verbatim de PLANTILLA §Script), co-ubicado con el subsistema en `.claude/planes/lint-planes/`. Es infra del Patrón, **no** una Herramienta: no se registra en `herramientas/INDICE.md` (decisión 0008).
6. **Asegurar el hook** `SessionStart` → `node .claude/planes/lint-planes/lint-planes.js --quiet` en `.claude/settings.json` (PLANTILLA §Hook). **Merge cuidadoso**: si el archivo o la clave `hooks` ya existen, agregar solo la entrada faltante sin pisar hooks ajenos; JSON válido (sin trailing commas).
